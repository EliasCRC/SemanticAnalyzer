	...
	/*Verificacion de herencia aciclica*/
        ...
        for (AbstractSymbol key : classHashMap.keySet()) {
         System.out.println(key);
  		...
  		classSet.clear();
  		classSet.add(key);
  		currClass = classHashMap.get(key);
  		while (!finished) {
  			if (classSet.contains(currClass.getParent())) {
  				semantError(currClass);
				errorStream.print("Cyclical inheritance\n");
  				finished = true;
  			} else {
  				if (isBasicClass(currClass.getParent())) {
  					finished = true;
  				} else {
  					classSet.add(currClass.getParent());
  					currClass = classHashMap.get(currClass.getParent());
  				}
  			}
  		}
	}
